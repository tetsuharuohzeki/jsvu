...
class Installer {
    ...
    installBinary(arg, options = { symlink: true }) {
        if (typeof arg === 'object') {
            for (const from of Object.keys(arg)) {
                const to = arg[from];
                installSingleBinary(`${this.sourcePath}/${from}`, `${this.targetPath}/${to}`);
                if (options.symlink) {
                    installSingleBinarySymlink(`${jsvuBinPath}/${to}`, `${this.targetPath}/${to}`);
                }
            }
        } else {
            const from = arg;
            installSingleBinary(`${this.sourcePath}/${from}`, `${this.targetPath}/${from}`);
            if (options.symlink) {
                installSingleBinarySymlink(`${jsvuBinPath}/${from}`, `${this.targetPath}/${from}`);
            }
        }
    }
    installBinarySymlink(arg) {
        if (typeof arg === 'object') {
            for (const to of Object.keys(arg)) {
                const from = arg[to];
                installSingleBinarySymlink(`${jsvuBinPath}/${from}`, `${this.targetPath}/${to}`);
            }
        } else {
            const to = arg;
            installSingleBinarySymlink(`${jsvuBinPath}/${to}`, `${this.targetPath}/${to}`);
    ...
    installLicense(arg) {
        for (const from of Object.keys(arg)) {
            ...
            fse.moveSync(`${this.sourcePath}/${from}`, to, {
                overwrite: true,
            });
        }
    }
    installScript({ name, alias, symlink, generateScript }) {
        ...
        const contents = generateScript(wrapperPath).trimStart().replaceAll('\t', '');
        fse.removeSync(to);
